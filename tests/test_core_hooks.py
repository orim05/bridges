...
def test_event_hooks():
    ...
    meta = bridge.register(add, params={"a": InputParamSource(), "b": InputParamSource()}, output=DisplayOutputDestination())
    meta({"a": "1", "b": "2"})
    assert events["pre"] and events["post"]
    meta_fail = bridge.register(fail, params={"a": InputParamSource()}, output=DisplayOutputDestination())
    try:
        meta_fail({"a": "1"})
    except Exception:
        pass
