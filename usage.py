...
class CLI(Interface):
        ...
        for key, value in config.items():
            if key == "global":
                ...
                if isinstance(value, dict):
                    ...
                    flow_config = (
                        CLIFlowConfig(**value.get("flow", {}))
                        if "flow" in value
                        else CLIFlowConfig()
                    )
                    layout_config = (
                        CLILayoutConfig(**value.get("layout", {}))
                        if "layout" in value
                        else CLILayoutConfig()
                    )
                    prompt_config = (
                        CLIPromptConfig(**value.get("prompt", {}))
                        if "prompt" in value
                        else CLIPromptConfig()
                    )
    ...
    def _show_help(self):
        ...
        for name, metadata in self.bridge.functions.items():
            ...
            aliases = (
                f" (aliases: {', '.join(custom.aliases)})"
                if custom and custom.aliases
                else ""
            )
            if self.global_config.colors:
        ...
        if self.global_config.colors:
            ...
            self.console.print(
                f"  {'/'.join(self.global_config.exit_commands)} - Exit", style="help"
            )
            self.console.print()
        else:
    ...
    def _collect_params_sequential(
        self,
    ...
    def _prompt_for_param(
        ...
        if isinstance(param_source, INPUT):
            ...
            while True:
                try:
                    choice = input(
                        f"\nSelect {param_name} (1-{len(options)}): "
                    ).strip()
    ...
    def _handle_output(
        self, result: Any, output: OutputDestination, custom: CLIFunctionCustomization
    ):
    ...
    cli.customize(
        {
            ...
            "global": {"title": "Calculator CLI", "theme": "default", "colors": True},
        }
    )
